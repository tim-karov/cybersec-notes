...
## 1. Recon & Enumeration
`nmap -sC -sV -v -T4 $ip`, получаем 2 открытых порта `22 (SSH)`, `80 (HTTP)`. Переходим на `http://ip:80`, нам открывается главная страница условного файлового хостинга, предполагаю, что тут мы будем заливать обратный шелл на сервер. В правом верхнем углу сразу видим кнопку логина, нажимаем и попадаем на форму ввода логина и пароля: `http://ip/login/login.php`. Попробовал дефолтные креды, ничего не подошло, идем дальше.
...
В домашнем каталоге находим пользователя john. А также вернемся к, ранее найденому, файлу `config.php`, который находится по пути: `/var/www/html/login`, читаем файл и находим креды админа. Облазив все вдоль и поперек, идея попробовать зареюзать пароль админа к пользаветелю john была последней, пробуем подключаться по ssh, как ни странно, но успех!
